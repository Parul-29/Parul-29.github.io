...
1. Push the `projects/agentic-ai-devsecops-v2` folder to its own GitHub
   repository, or import the parent repository and select this folder as
   **Root Directory** in Vercel.
2. In Supabase SQL Editor, create the `scans` table using the SQL in
   [SUPABASE_SETUP.md](SUPABASE_SETUP.md).
3. In Vercel, import the repository. It will use `vercel.json` and deploy the
   FastAPI entry point at `api/index.py` automatically.
4. In **Settings → Environment Variables**, add these values for Production
   (and Preview if required):
   ...
   Keep the service-role key only in Vercel; never commit it or expose it in a
   browser application.
5. Redeploy, then verify `https://your-vercel-domain.vercel.app/health` returns
   `{"status":"ok"}`. Your API endpoints are available at `/api/v1/...`.
